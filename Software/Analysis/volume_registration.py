...
def findOutlines(image):
    
    # converted from Matlab code in https://github.com/cortex-lab/allenCCF
    #
    # Shamash P, Carandini M, Harris KD, Steinmetz NA (2018) bioRxiv
    # https://doi.org/10.1101/447995
    #
    
    diffilt = -1/8*np.ones((3,3));
    diffilt[1,1] = 1;
    thisd = convolve2d(image, diffilt, mode='same')
    thisbw = np.abs(np.sign(thisd))
    
    return binary_dilation(thisbw).astype('int')*255
        
def scaleCCFImage(image):
    
    new_size = (590,940)
        
    slc = resize(image, new_size, preserve_range=True)
    
    V = np.zeros((1024,1024))
    
    xwidth = slc.shape[0]
    ywidth = slc.shape[1]
    xpad = int((1024 - xwidth)/2)
    ypad = int((1024 - ywidth)/2)
    
    V[xpad:1024-xpad,ypad:1024-ypad]  = slc
...
for slice_i, test_slice in enumerate(test_slices): 
    ...
    for zi in range(test_slice,test_slice+3): #vol2.shape[2]):
        ...
        for xi in range(vol2.shape[1]):
            for yi in range(vol2.shape[2]):
                ...
                ccf_coordinate = (np.array([1023-z,x,y]) - origin) * scaling
    ...
    slice_num = int(ccf_coordinate[0])

    im = labels[slice_num,:,:]
    
    overlay = scaleCCFImage(findOutlines(im))
    
    this_image = np.max(vol2[:,:,:],0)
    xx,yy = np.where(overlay > 0)
    this_image[xx,yy] = 100

    plt.subplot(2,2,slice_i + 1)
